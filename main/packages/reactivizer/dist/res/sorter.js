    ...
    const sortActions = {
        ...
        async sort(buf, offset = 0, len, noForkThreshold = 50) {
            ...
            if (arr.length > noForkThreshold) {
                ...
                const forkDone = rx.lastValueFrom(rx.merge(sorter.i.pt.sortResolved.pipe((0, control_1.actionRelatedToPayload)(sortAction.i), rx.map(([, res]) => res), 
                // Wait for sortCompleted recieved, so that Jest test won't report console log after test exits
                rx.takeUntil(sorter.i.pt.sortCompleted.pipe((0, control_1.actionRelatedToPayload)(sortAction.i))), (0, utils_1.timeoutLog)(6000, () => o.dp.warn('fork sort timeout', rightPartOffset, rightpartLen, 'action id:', sortAction.i)))));
