...
export function createWorkerControl(opts) {
    ...
    r('"fork" -> mainPort.postMessage, forkByBroker', o.at.fork.pipe(rx.switchMap(a => outputTable.l.workerInited.pipe(rx.map(b => [a, b]), rx.take(1))), rx.mergeMap(([forkAction, [, , , mainPort]]) => {
        ...
        return rx.merge(rx.fromEventPattern(h => chan.port1.on('message', h), h => {
            ...
            chan.port1.close();
        }).pipe(rx.map(event => deserializeAction(event, i)), rx.take(1), rx.takeUntil(rx.merge(error$, close$))), error$.pipe(rx.tap(err => o.dpf._onErrorFor(wrappedAct, err))), new rx.Observable(_sub => {
            if (mainPort) {
