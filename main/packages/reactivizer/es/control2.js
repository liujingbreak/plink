...
class SingleActionFactoryImpl {
    ...
    do(waitForAction$, referActionMeta) {
        ...
        rx.merge(this.control.doOperator$.pipe(rx.take(1), rx.switchMap(operator => waitForAction$.pipe(operator(action), actionRelatedToAction(action), mapActionToPayload()))), new rx.Observable(sub => {
