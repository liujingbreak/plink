...
export interface ActionOfUnknownType<P extends any[]> {i: ActionMeta['i']; r: ActionMeta['r']; p: P}
...
export interface SingleActionFactory {
   ...
   * */
  od<T extends [ActionMeta, ...any[]] | Action<any>, TA extends ([ActionMeta, ...any[]] | Action<any>)[]>(
   ...
   */
  odMono<T extends [ActionMeta, ...any[]] | Action<any>, TA extends ([ActionMeta, ...any[]] | Action<any>)[]>(
    response$: rx.Observable<T>, ...moreResponses: [...{[K in keyof TA]: rx.Observable<TA[K]>}]
  ): TA['length'] extends 0 ? rx.Observable<T> : [rx.Observable<T>, ...{[K in keyof TA]: rx.Observable<TA[K]>}];
}
...
export class SingleActionFactoryImpl<I, K extends keyof I> implements SingleActionFactory {
  ...
  od<T extends [ActionMeta, ...any[]] | Action<any>, TA extends ([ActionMeta, ...any[]] | Action<any>)[]>(
    ...
    if (moreResponses.length === 0) {
      ...
      const responses = [response, ...moreResponses] as const;
      ...
      return onAllSubscribed(
        (responses as readonly rx.Observable<T | TA[number]>[]).map(r => this.control.doOperator$.pipe(
          ...
          timeoutLog(
            this.opts.slowDispatchObservableTime ?? 20000,
            this.opts.slowLog ? () => {this.opts.slowLog!(this.action);} : () => {
