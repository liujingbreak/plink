...
export const reg = new FinalizationRegistry(() => {
    countReclaimed++;
...
async function test() {
    ...
    const done = rx.firstValueFrom(rx.merge(onFinalize$.asObservable().pipe(rx.timeout(20000), rx.take(NUM), rx.map((claimedCount, i) => {
        ...
        console.error(err);
        throw err;
    })), rx.timer(0, 1000).pipe(rx.map((t) => console.log(`${t}s count reclaimed: ` + countReclaimed, 'allRefs:', allRefs.size, 'first item', allRefs.size < 5 ? [...allRefs.values()].map(item => { var _a; return (_a = item.deref()) === null || _a === void 0 ? void 0 : _a.logPrefix; }) : '')), rx.ignoreElements()), rx.timer(30000) // wait for 30s at most, if there is no timer, Node.js will exit due to event loop become empty before GC taking place
    ).pipe(rx.take(1)));
    items = [];
    console.log('size of Configurables:', formater.format(allRefs.size));
