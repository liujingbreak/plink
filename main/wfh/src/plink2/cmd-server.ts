...
import {createProcessService} from './server-process';
...
const processService = createProcessService(reatorLog);
...
r('processService.onChildProcessExit', processService.destory$.pipe(
...
r('onRequestLine', o.pt.onRequestLine.pipe(
  rx.mergeMap(([m, , res, line]) => {
    ...
    return processService.i.ft.sendCommand(inputTable.getData().setTTYSize as [number, number], dir, args, out).ddo(
      processService.o.pt.onCommandDoneAnyway
    ).pipe(
      rx.take(1),
      rx.finalize(() => {
