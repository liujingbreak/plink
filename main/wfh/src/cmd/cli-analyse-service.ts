...
export interface AnalyseInput {
  forkDfsTraverseFiles(files: string[], tsconfigFile: string | null | undefined,
    alias: [reg: string, replaceTo: string][], ignore?: string): SingleActionFactory;

  dfsTraverseFiles(files: string[], tsconfigFile: string | null | undefined,
    alias: [reg: string, replaceTo: string][], ignore?: string): SingleActionFactory;
...
export function createService() {
  ...
  r('forkDfsTraverseFiles -> doneDfsTraverseFiles', i.pt.forkDfsTraverseFiles.pipe(
    rx.mergeMap(async ([m, ...args]) => {
      ...
      const [, result] = await setIdleDuring.asPromise(service, done$);
      o.ft.doneDfsTraverseFiles(result).dp(m);
    })
  ));

  r('dfsTraverseFiles -> doneDfsTraverseFiles', i.pt.dfsTraverseFiles.pipe(
    rx.tap(([m, files, tsconfig, alias, ignore]) => {
      const res = dfsTraverseFiles(files, tsconfig, alias, ignore);
      o.ft.doneDfsTraverseFiles(res).dp(m);
    })
  ));
  return service;
}

function dfsTraverseFiles(files: string[], tsconfigFile: string | null | undefined,
