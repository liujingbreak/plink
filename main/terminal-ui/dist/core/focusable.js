...
export const focusServiceFac = new BaseReactorFactory({
    ...
    const focusableTree$ = createRtreeInstance();
    ...
    r('removeFocusable', pt.removeFocusable.pipe(rx.mergeMap(([m, c]) => focusableTree$.pipe(rx.take(1), rx.map(rtree => {
    ...
    // maintain tree
    r('onRectChange -> "xTree","yTree","rightXTree","bottomYTree"', pt.onRectChange.pipe(rx.mergeMap(a => focusableTree$.pipe(rx.take(1), rx.map(b => [a, b]))), rx.withLatestFrom(canvas.latest.setBounding), rx.map(([[[, rect, c], rtree], [, , , , canHeight]]) => {
        var _a;
    ...
    r('focusOnComponent -> didFound,rootFocus.eventHandlingSvc,rootFocus.switchFocus', pt.focusOnComponent.pipe(rx.switchMap(([m, comp]) => {
        return comp.ft.queryContext(ROOT_FOCUS_SERVICE_CONTEXT).re(m).od(comp.pt.onContextChange).pipe(rx.take(1), rx.mergeMap(([, , rootFocus]) => {
            if (!rectByComponent.has(comp)) {
                // wait for "onRectChange" being handled
                return pt.onRectChange.pipe(rx.filter(([, , c]) => c === comp), rx.take(1), rx.map(() => {
                    return rootFocus;
                }));
            }
            return rx.of(rootFocus);
        }), rx.mergeMap(rootFocus => {
            return ft._emitDidFoundUpToRoot(comp, rootFocus).re(m).od(pt._didEmitDidFoundUpToRoot).pipe(rx.take(1), rx.map(() => {
                rootFocus.ft.eventHandlingSvc(service).dp(m);
                rootFocus.ft.switchFocus(service, comp.getLogName(), comp).dp(m);
            }));
        }));
    })));
    r('_emitDidFoundUpToRoot -> didFound,_didEmitDidFoundUpToRoot', pt._emitDidFoundUpToRoot.pipe(rx.mergeMap(([m, comp, rootSvc]) => {
        ...
        return latest.forRootComp.pipe(rx.switchMap(([, rootComp]) => rootComp.latest.focusService.pipe(rx.switchMap(([, pf]) => {
            return pf.ft._emitDidFoundUpToRoot(rootComp, rootSvc).re(m).od(pf.pt._didEmitDidFoundUpToRoot).pipe(rx.take(1), rx.map(() => { ft._didEmitDidFoundUpToRoot().dp(m); }));
        }), rx.take(1))));
    })));
    ...
    r('findFocusable,didFound,didNotFound,handleKeyEvents... -> onFocus,rootFocus.switchFocus', pt.findFocusable.pipe(rx.switchMap(([m, dir]) => {
        return forked.pt.didFound.pipe(actionRelatedToAction(m), rx.takeUntil(forked.pt.didNotFound.pipe(actionRelatedToAction(m), rx.mergeMap(([m2]) => latest.forRootComp.pipe(rx.mergeMap(([, root]) => rx.combineLatest([
            root.latest.focusService,
            root.ft.queryContext(ROOT_FOCUS_SERVICE_CONTEXT).re(m)
                .od(root.pt.onContextChange).pipe(rx.take(1))
        ]).pipe(rx.map(([[, pf], [, , rootFocus]]) => {
            rootFocus.ft.eventHandlingSvc(pf, dir).dp(m2, m);
        }))))))), rx.take(1), 
        ...
        rx.mergeMap(([, rect, c]) => {
            ...
            return c.ft.queryContext(ROOT_FOCUS_SERVICE_CONTEXT).re(m)
                .od(c.pt.onContextChange).pipe(rx.take(1), rx.map(([, , rootFocus]) => [c, r, rootFocus]));
            ...
            if (r) {
                ...
                return c.ft.queryContext('focusSvc').re(m).od(c.pt.onContextChange).pipe(rx.filter(([, , v]) => v != null), rx.take(1), rx.map(([, , v]) => {
    ...
    r('findFocusable,didFound -> didFound, didNotFound', pt.findFocusable.pipe(rx.withLatestFrom(rx.merge(latest.didFound, pt.didNotFound.pipe(rx.map(([m]) => [m, null, null, null])))), rx.mergeMap(a => focusableTree$.pipe(rx.take(1), rx.map(b => [...a, b]))), rx.map(([[m, dir, handleEventAct], [, lastRect, lastComp, tabIdx], rtree]) => {
        var _a, _b, _c, _d, _e;
        service.log('-- findFocusable, lastComp', lastComp === null || lastComp === void 0 ? void 0 : lastComp.getLogName());
        if (dir === SearchDirection.tabNext) {
            if (lastRect == null || lastComp == null || !rectByComponent.has(lastComp)) {
                ...
                [lastRect, tabIdx] = (_a = rectByComponent.get(lastComp)) !== null && _a !== void 0 ? _a : [undefined, undefined];
                if (tabIdx == null) {
            ...
            if (idx >= 0 && idx < node.value.length - 1) {
                ...
                const [r, t] = (_b = rectByComponent.get(c)) !== null && _b !== void 0 ? _b : [undefined, undefined];
            ...
            const nextCompPos = (_c = rectByComponent.get(nextComp)) !== null && _c !== void 0 ? _c : [undefined, undefined];
            ...
            if (lastRect == null) {
                ...
                if (chosen) {
                    const [r, t] = (_d = rectByComponent.get(chosen)) !== null && _d !== void 0 ? _d : [undefined, undefined];
                    ft.didFound(r, chosen, t).dp(m);
                    return;
                }
            }
            else {
                const bigger = [...lastRect];
                ...
                if (chosen) {
                    const [r, t] = (_e = rectByComponent.get(chosen)) !== null && _e !== void 0 ? _e : [undefined, undefined];
                    ft.didFound(r, chosen, t).dp(m);
                    return;
                }
            }
    ...
    function findNextComp(rtree, currRect, dir, bboxOfAll) {
        if (currRect == null) {
            switch (dir) {
                case SearchDirection.up:
...
const tableForRoot = ['switchFocus', 'eventHandlingSvc'];
export const rootFocusSvcFac = focusServiceFac.forExtend({
    ...
    r('eventHandlingSvc', latest.handleKeyEvents.pipe(rx.take(1), rx.switchMap(([, eventSvc]) => latest.eventHandlingSvc.pipe(rx.distinctUntilChanged(([, a], [, b]) => a === b), rx.scan((prev, curr) => {
    ...
    r('switchFocus, didNotFound -> comp.focus', pt.switchFocus.pipe(rx.filter(([, , , c]) => c != null), rx.switchMap(([m0, , , c]) => pt.didNotFound.pipe(rx.map(([m]) => {
