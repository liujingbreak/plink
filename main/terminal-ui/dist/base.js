...
exports.baseComponentFac = new reactivizer_1.BaseReactorFactory({
    ...
    r('needRerender, ofCanvas -> canvas.requestRender', pt.needRerender.pipe(rx.filter(([, need]) => need), rx.switchMap(([m]) => latest.onDetached.pipe(rx.take(1), rx.filter(([, detached]) => !detached), rx.map(() => m))), rx.switchMap(m => latest.ofCanvas.pipe(rx.map(([, canvas]) => {
        if (canvas)
    ...
    r('setRenderChanges -> needRerender', pt.setRenderChanges.pipe(rx.switchMap(([, list]) => rx.merge(list.map(it => it.pipe(rx.skip(1))))), rx.mergeMap(o => o), rx.map(([m]) => ft.needRerender(true).dp(m))));
