...
export const flexContainerFac = baseContainerFac.forExtend<FlexContainerEvents, typeof tableForFlexContainer>({
  ...
  r('reflow,onChildPositions... -> "childBoundingRTree$.insert..."', pt.reflow.pipe(
    rx.switchMap(([m]) => {
  ...
  r('reflow, ... -> onChildPositions, child.onSize, onChangeChildrenSize', listContainer.pt.reflow.pipe(
    ...
    rx.switchMap(([
      ...
      const setPositions$ = new rx.Observable<void>(sub => {
        ...
        for (let i = 0, l = children.length; i < l; i++) {
          ...
          if (dir === 'row')
            children[i].ft.onSize(chrMainAxisSizes[i] ?? 0, chrCrossAxisSizes[i] ?? 0).dp(m);
          else
            children[i].ft.onSize(chrCrossAxisSizes[i] ?? 0, chrMainAxisSizes[i] ?? 0).dp(m);
        }
      ...
      return rx.concat(
        calcChdSizes$.pipe(
          rx.finalize(() => ft.onChangeChildrenSize(chrMainAxisSizes, chrCrossAxisSizes).dp(m))
        ),
        setPositions$
      ).pipe(
  ...
  r('findOverlaps -> didFindOverlaps', pt.findOverlaps.pipe(
    rx.mergeMap(([m, ...rect]) => table.l.onBoundingBox.pipe(
      ...
      rx.withLatestFrom(childBoundingRTree$),
      rx.mergeMap(([[x, y, w, h], cbt]) => {
  ...
  r('allDisplayChildren,...-> requestReflow', pt.allDisplayChildren.pipe(
    rx.switchMap(([, chdn]) => {
      return rx.merge(...chdn.map(
        chd => rx.merge(chd.pt.setFlexGrow, chd.pt.setFlexShrink)
      ));
    }),
