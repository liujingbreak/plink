...
export const canvasCacheFac = new BaseReactorFactory<CanvasCacheEvents, typeof tableFor>({
  ...
  r('addStr -> add', s.pt.addStr.pipe(
    rx.map(([m, x, y, str, style]) => s.ft.add(x, y, [...getTextDisplayUnits(str)], style ?? []).dp(m))
  ));
          ...
          for (const [l, h, data] of existings) {
            ...
            if (h > newCRight) {
              if (data === 'clear') {
                ...
                const choppedUnits = units.slice(
                  prevCharFullWidth ? cRight - l + 1 : cRight - l);
                const node = line.insertInterval(
                  prevCharFullWidth ? cRight + 1 : cRight, h);
                node.value = [choppedUnits, style];
              }
            }
          }
  ...
  r('fetchItems -> onRenderItem,onClearItem,didFetchItems', s.pt.fetchItems.pipe(
    rx.map(([m, rel]) => {
  ...
  r('cleanup', s.pt.cleanup.pipe(
    rx.map(() => {
      cache = new RedBlackTree<number, CacheLine>();
    })
  ));
  s.ft.cache(cache).dp();
});
