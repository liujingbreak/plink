...
async function createCommands(argv, manualExitProcess) {
...
function subComands(program, manualExitProcess) {
    ...
    if (manualExitProcess) {
        ...
        const initCmd = program.command('init').alias('sync')
            ...
            // eslint-disable-next-line no-console
            console.log((0, misc_1.sexyFont)('PLink').string);
            require('./cli-init').default(initCmd.opts(), workspace);
        });
    ...
    program.command('project [add|remove] [dir...]')
        ...
        // eslint-disable-next-line no-console
        console.log((0, misc_1.sexyFont)('PLink').string);
        await require('./cli-project').default({ isSrcDir: false }, action, projectDir);
    });
    program.command('src [add|remove] [dir...]')
        ...
        // eslint-disable-next-line no-console
        console.log((0, misc_1.sexyFont)('PLink').string);
        await require('./cli-project').default({ isSrcDir: true }, action, dirs);
    });
    /**
     * command clean
     */
    program.command('cs').alias('clear-symlinks')
        ...
        const editor = require('../editor-helper');
    ...
    const upgradeCmd = program.command('upgrade')
        ...
        .action(async () => {
        skipVersionCheck = true;
        await require('./cli-link-plink').reinstallWithLinkedPlink(upgradeCmd.opts());
    });
    ...
    const listCmd = program.command('ls').alias('list')
        ...
        .action(async () => {
        await require('./cli-ls').default(listCmd.opts());
    });
    const addCmd = program.command('add <dependency...>')
        ...
        await require('./cli-add-package').addDependencyTo(packages, addCmd.opts().to, addCmd.opts().dev);
    });
    const tsconfigCmd = program.command('tsconfig')
        ...
        .action(() => {
        require('./cli-tsconfig-hook').doTsconfig(tsconfigCmd.opts());
    });
    ...
    const bumpCmd = program.command('bump [package...]')
        ...
        .action(async (packages) => {
        await require('./cli-bump').default(Object.assign(Object.assign({}, bumpCmd.opts()), { packages }));
    });
    ...
    const packCmd = program.command('pack [package...]')
        ...
        .action(async (packages) => {
        await require('./cli-pack').pack(Object.assign(Object.assign({}, packCmd.opts()), { packages }));
    });
    ...
    const publishCmd = program.command('publish')
        ...
        .action(async (packages) => {
        await require('./cli-pack').publish(Object.assign(Object.assign({}, publishCmd.opts()), { packages }));
    });
    const analysisCmd = program.command('analyze')
        ...
        .action((packages) => {
        return require('./cli-analyze').default(packages, analysisCmd.opts());
    });
    ...
    const updateDirCmd = program.command('update-dir')
        ...
        .action((_workspace) => {
        require('./cli-ls').checkDir(updateDirCmd.opts());
    });
    if (manualExitProcess) {
...
function spaceOnlySubCommands(program) {
    ...
    program.command('setting [package]')
        ...
        .action((pkgName) => {
        require('./cli-setting').default(pkgName);
    });
    /** command run*/
    const runCmd = program.command('run <target> [arguments...]')
        .description('Run specific module\'s exported function\n')
        .action(async (target, args) => {
        await require('../package-runner').runSinglePackage({ target, args });
    });
...
function loadExtensionCommand(_program, ws, overrider) {
    if (ws == null)
        return [];
