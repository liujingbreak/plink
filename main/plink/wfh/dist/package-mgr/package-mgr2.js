...
r('onProjectLinked, onDirLinked, rootPackageJson -> didScanSource, addPackageToProject', rx.from(import('../recipe-manager.js')).pipe(rx.mergeMap(rm => i.pt.updatePackagesBegin.pipe(rx.mergeMap(([m]) => outputTable.l.rootDir.pipe(rx.map(([, rootDir]) => [m, rootDir]))), rx.switchMap(([m, rootDir]) => rx.forkJoin([
    o.pt.onProjectLinked.pipe((0, reactivizer_1.actionRelatedToActionRelatives)(m), rx.takeUntil(i.pt.updatePackagesEnd.pipe((0, reactivizer_1.actionRelatedToActionRelatives)(m))), rx.mergeMap(async ([, dir]) => {
        const projPkgJson = JSON.parse(await node_fs_1.default.promises.readFile(node_path_1.default.resolve(rootDir, dir, 'package.json'), 'utf8'));
        if (projPkgJson.workspaces) {
            for (const pkgPath of projPkgJson.workspaces) {
                const info = (0, package_mgr2_utils_1.createPackageInfo)(node_path_1.default.resolve(dir, pkgPath, 'package.json'), false);
                if (info.json.plink)
    ...
    o.pt.onDirLinked.pipe((0, reactivizer_1.actionRelatedToActionRelatives)(m), rx.takeUntil(i.pt.updatePackagesEnd.pipe((0, reactivizer_1.actionRelatedToActionRelatives)(m))), rx.reduce((arr, [, dirPattern]) => {
        arr.push(dirPattern);
        return arr;
    }, []), rx.map(dirs => {
...
r('updatePackagesEnd, data_allPackages, updateDependencyOfSpace, data_spaceDependencyMap -> addPackageToSpace, deletePackageOfSpace, didSyncSpacePackages', rx.combineLatest([
    i.pt.updatePackagesEnd.pipe(rx.mergeMap(() => outputTable.l.data_allPackages.pipe(rx.take(1)))),
...
r('switchToSpace, didSyncSpacePackages -> checkSpace, createOrChangeSymlink, didSwitchSpace, writeFile', i.pt.switchToSpace.pipe(rx.filter(([, key]) => key != null), rx.distinctUntilChanged(), rx.concatMap(([m, spaceKey]) => {
    ...
    rx.from(pkgSet !== null && pkgSet !== void 0 ? pkgSet : []).pipe(rx.mergeMap(async (pkgName, idx) => {
        ...
        if (it)
            acc.push(it);
        return acc;
    }, []), rx.mergeMap(a => npmInstallSpacePkgJsonTable.latestPayloadOf(key).pipe(rx.take(1), rx.map(b => [a, b]))), rx.mergeMap(async ([workspaces, [, , json]]) => {
        var _a;
